...
internal sealed class ClickHouseTcpConnection : IDisposable, IAsyncDisposable
{
    ...
    /// <exception cref="ClickHouseTcpConnectionException">The socket could not connect, or the TLS handshake failed (certificate rejected, or the port is not a TLS port). The cause is the inner exception.</exception>
        ...
        TimeSpan readTimeout = default)
    {
        ...
        {
            socket.Dispose();
            throw ConnectionFailure.Connect(host, port, e);
        }
        catch
        {
            socket.Dispose();
    ...
    /// (protocol violation, connection error, cancellation) terminates the connection before propagating, so

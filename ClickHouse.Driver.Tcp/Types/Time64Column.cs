...
/// <summary>
/// A <c>Time64(scale)</c> column that surfaces the raw wire counts. ClickHouse writes a little-endian
/// <c>Int64</c> tick count at <c>10^-scale</c> seconds per row (a signed time-of-day/duration, not tied to a
/// date); those bytes are kept verbatim as the column's storage and exposed — with no copy — as the
/// <see cref="long"/> <see cref="Values"/>, the exact wire value at any scale (including scales 8 and 9, which
/// are finer than a .NET tick). The scale — the same for every row — lives once on the column.
///
/// <para>
/// A caller that wants a <see cref="TimeSpan"/> can call <see cref="GetTimeSpan"/> or <see cref="ToTimeSpans"/>;
/// that projection is at .NET's 100 ns resolution and is therefore lossy for scales finer than 7 — read
/// <see cref="Values"/> for the exact count.
/// </para>
///
/// <para>
...
/// </para>
/// </summary>
internal sealed class Time64Column : IColumn<long>, ITimeColumn, IStoredValuesColumn
{
    ...
    public Time64Column(string name, string typeName, int scale, byte[] buffer, int length, bool pooled)
    {
        Name = name;
        TypeName = typeName;
        this.scale = scale;
    ...
    /// <summary>
    /// The raw signed tick counts (at <c>10^-Scale</c> seconds), as a zero-copy view. This is the exact wire
    /// value at every scale; use <see cref="ToTimeSpans"/> for a (lossy) <see cref="TimeSpan"/> view.
    /// </summary>
    ...
    /// <summary>Returns a row as a <see cref="TimeSpan"/> at 100 ns resolution.</summary>
    /// <param name="row">The zero-based row index.</param>
    /// <returns>The duration. Lossy for scales finer than 7 (sub-100 ns digits are truncated); read
    /// <see cref="Values"/> for the exact count.</returns>
    public TimeSpan GetTimeSpan(int row) => ToTimeSpan(Values[row]);
    ...
    /// <remarks>A <see cref="TimeSpan"/> holds only 100 ns ticks, so this view is lossy for scales finer than 7
    ...
    public TimeSpan[] ToTimeSpans()
    {
        ...
        var result = new TimeSpan[counts.Length];
        for (int i = 0; i < counts.Length; i++)
        {
            result[i] = ToTimeSpan(counts[i]);
        }
    ...
    /// <summary>
    /// Reads a <c>Time64</c> column: bulk-reads the raw count bytes into a pooled buffer that becomes the column's
        ...
        CancellationToken cancellationToken)
    {
        if (rowCount == 0)
        {
            return new Time64Column(name, typeName, scale, Array.Empty<byte>(), length: 0, pooled: false);
        }

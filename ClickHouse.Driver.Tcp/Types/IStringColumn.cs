...
/// filled from a <c>String</c> column the same way. Those copy a row at a time; this view is the zero-copy form,
/// and the only one that reaches the whole blob at once.
/// </para>
///
/// <para>
/// Every wrapper forwards the reading to the child column holding the bytes, so it composes: a
/// <c>Nullable(String)</c> reads as a <c>byte[]</c> with its NULL rows null, an <c>Array(String)</c> as a
/// <c>byte[][]</c> per row, a <c>Map(K, String)</c> and a <c>Tuple(..., String)</c> likewise, and a
/// <c>LowCardinality(String)</c> as one array per <em>dictionary entry</em>, shared by every row holding that key
/// — so do not mutate an array read from one. To reach the same child columns by hand, and without a copy, match
/// <see cref="ILowCardinalityColumn"/>, <see cref="IArrayColumn"/> or <see cref="IMapColumn"/> and then match the
/// child as this interface.
/// </para>
/// </summary>

...
public class ReadBufferTests
{
    ...
    public void EnsureAsync_StreamEndsEarly_Throws()
    {
        ...
        Assert.ThrowsAsync<ClickHouseTcpConnectionException>(async () => await buffer.EnsureAsync(8, None));
    }
    ...
    [Test]
    public void EnsureAsync_SocketFails_WrapsTheCauseAsAConnectionFailure()
    {
        using var buffer = new ReadBuffer(new ThrowingStream(new IOException("connection reset by peer")), 64);

        var thrown = Assert.ThrowsAsync<ClickHouseTcpConnectionException>(async () => await buffer.EnsureAsync(8, None));
    ...
    // relabelled as a connection failure the caller should retry.
    [Test]
    ...
    {
        using var buffer = new ReadBuffer(new MemoryStream(new byte[] { 1, 2, 3 }), 64);
        Assert.ThrowsAsync<ClickHouseTcpConnectionException>(async () => await buffer.ReadIntoAsync(new byte[8], None));
    }

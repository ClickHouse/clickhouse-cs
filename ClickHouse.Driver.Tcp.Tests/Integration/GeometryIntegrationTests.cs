...
///
/// <para>
/// The round-trip corpus cannot pin it for four of the six: <c>Ring</c> and <c>LineString</c> are both
/// <c>Array(Point)</c>, as are <c>Polygon</c> and <c>MultiLineString</c>, so transposing a pair emits a
/// byte-identical block and reads back identical CLR values. Nor is it enough to write discriminator <c>i</c> and
/// ask the server what it calls <c>i</c> — that answers from the server's own list and never consults the
/// client's. So the expectation here is read out of the codec: the client's name for each discriminator is
/// compared with the server's name for the row written against it.
/// </para>
/// </summary>
[TestFixture]

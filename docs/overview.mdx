...
**The server, not the client, picks the codec.** ClickHouse scans `Accept-Encoding` for tokens in its own fixed preference order — `zstd` > `br` > `lz4` > `snappy` > `gzip` > `deflate` — and ignores both the order you list them in and any q-values. So the header is a capability announcement rather than a demand, and the only way to steer the choice is which tokens you leave out. The default names `zstd` first in that scan, so a default query is answered with zstd; the remaining tokens are the fallback for a server or intermediary that cannot. `br` is *decodable but not advertised*: it outranks every fallback token in the default list, so naming it would make it the codec for every query the server cannot answer with zstd.

How the codecs compare on payload size, server CPU and client CPU depends on your data, your link and the server's `http_zlib_compression_level` (shipped default: 3), so two cases are worth knowing about — in both of them asking for `lz4` explicitly may serve you better:

- **`http_zlib_compression_level=1`.** That setting applies to every HTTP codec, not just zlib, and it changes the picture: `lz4` only engages LZ4-HC from level 3, so its server-side cost at level 1 is not what the default level would suggest. If your server runs level 1, measure before relying on the default.
- **A CPU-bound client on a fast link.** The driver decodes the response body on the calling thread, so when the network is not the bottleneck it is the client-side decode cost that decides.

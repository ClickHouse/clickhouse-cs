...
for one key, so `JsonReadMode.Binary` throws a `SerializationException` naming the two paths.

This applies only where both paths hold a value in that row. Where the overlap is declared with
`Nullable` types, the row usually fills in one side only — `JSON(a Nullable(Int64), a.b
Nullable(Int64))` reads `{"a":5}` and `{"a":{"b":7}}` normally — so nothing throws.

...
zstd is also available for the **request** side of a binary insert, through `InsertOptions.Compressor`, alongside `Lz4Compressor`, `GZipCompressor` and `BrotliCompressor`. Inserts are *not* zstd by default — they still use `gzip:Fastest` — so this one is opt-in. `ZstdCompressor.Default` uses zstd's own default level (3); the constructor takes any level between `ZstdCompressor.MinLevel` and `ZstdCompressor.MaxLevel` — zstd's levels are a signed range, and negative ones trade ratio for speed.

```csharp
var options = new InsertOptions { Compressor = ZstdCompressor.Default };  // Content-Encoding: zstd
await client.InsertBinaryAsync("events", columns, rows, options);
```

Both codecs are built in: `ClickHouse.Driver` bundles vendored, pure-managed copies of the LZ4 and zstd codecs, so neither adds a runtime dependency.

...
| Property | Type | Default | Connection String Key | Description |
|----------|------|---------|----------------------|-------------|
| UseCompression | `bool` | `true` | `Compression` | Ask the server to compress query responses (`enable_http_compression`); see `AcceptEncoding` for the codec. Does not affect request bodies — see [Insert compression](#insert-compression) |
...
{
    AcceptEncoding = "br",      // better ratio than lz4, more server CPU
});
```
...
**The server, not the client, picks the codec.** ClickHouse scans `Accept-Encoding` for tokens in its own fixed preference order — `zstd` > `br` > `lz4` > `snappy` > `gzip` > `deflate` — and ignores both the order you list them in and any q-values. So the header is a capability announcement rather than a demand, and the only way to steer the choice is which tokens you leave out. The default names `zstd` first in that scan, so a default query is answered with zstd; the remaining tokens are the fallback for a server or intermediary that cannot. `br` is *decodable but not advertised*: it comes earlier still in the scan and is by far the dearest codec on both sides, so naming it would make it the codec for **every** query.

`zstd` is the default because at the shipped `http_zlib_compression_level` (3) it wins on all three axes at once against `lz4` — measured on real data (a 15-column ClickBench `hits` subset) over a real network, against both a 4-vCPU server and a 16-vCPU ClickHouse Cloud service: 22–26% fewer bytes, 40–51% less server CPU, and 19–59% less wall-clock fetch time.

Two cases are worth knowing about, because in both of them `lz4` is the better choice and you should ask for it explicitly:

- **`http_zlib_compression_level=1`.** That setting applies to every HTTP codec, not just zlib, and `lz4` is unusually sensitive to it — it only engages LZ4-HC (and gets expensive) from level 3. At level 1, `zstd` costs **10–18% more server CPU** than `lz4`. The clean sweep above is a property of the default level, not of the codecs.
- **A CPU-bound client on a fast link.** `zstd` is the slower codec to *decode* — roughly 0.7–1.4 GB/s against `lz4`'s 1.1–2.3 GB/s (`gzip`: 0.2–0.3) — and the driver decodes on the calling thread. When the network is not the bottleneck, `lz4` finishes sooner.
...
using var reader = await client.ExecuteReaderAsync(
    "SELECT * FROM events",
    options: new QueryOptions { AcceptEncoding = "lz4" });   // cheaper to decode than the zstd default
```
...
zstd is also the default on the **request** side of a binary insert, through `InsertOptions.Compressor` (alongside `Lz4Compressor`, `GZipCompressor` and `BrotliCompressor`). It replaced `gzip:Fastest` there because `zstd:3` beats it on both axes that are deterministic client-side: ~32% smaller request bodies for ~64% less client CPU. `ZstdCompressor.Default` uses zstd's own default level (3); the constructor takes any level between `ZstdCompressor.MinLevel` and `ZstdCompressor.MaxLevel` — zstd's levels are a signed range, and negative ones trade ratio for speed.

One caveat is specific to the request side. `Accept-Encoding` is an *offer* the server may decline, but a request body's `Content-Encoding` is a **declaration**: an intermediary that re-encodes request bodies and does not understand zstd fails the insert outright rather than merely slowing it down. ClickHouse itself accepts zstd request bodies on every supported version (verified back to 25.8 LTS); if your inserts traverse a proxy tier you are unsure of, name the codec explicitly.
...
Every codec named here is built in: `ClickHouse.Driver` bundles vendored, pure-managed copies of the LZ4 and zstd codecs (gzip, deflate and brotli come from the BCL), so none of them adds a runtime dependency.

...
link is relative to how fast the codec runs, and the answer flips completely between a
same-datacenter deployment and one crossing the internet. There is no setting that is right for
everyone.
...
The driver is fast with its default settings. Use this section when you have a specific problem: a
scan that allocates too much, an insert that is slower than you expect, or a latency figure you
cannot explain.

The items are in order of how much they usually give you.

### At a glance {#perf-at-a-glance}

| If you | Do this | Typical effect |
|---|---|---|
| read rows into your own classes | Use [`QueryAsync<T>`](#perf-read-path), not `MapTo<T>` | 56% less allocation |
| insert more than a few thousand rows | Increase [`InsertOptions.BatchSize`](#perf-insert-batching) | Up to 49% less time |
| run an insert-heavy console or worker app | Turn on [Server GC](#perf-gc) | Up to 49% less time |
| read large results across a network | Keep response compression on (the default) | 16% to 33% less time |
| insert across a fast link | Try [`InsertOptions.Compressor = null`](#perf-compression) | 4% to 30% less time |
| insert into the same table many times | Use [`UseSchemaCache` or `ColumnTypes`](#skip-schema-query) | One less round trip per insert |
| read very large results | Increase [`ReadBufferSize`](#perf-buffers) | Fewer buffer refills |

<Note>
These figures come from the driver's macro-benchmark suite, measured on 1,000,000 rows of the
ClickBench `hits` table (105 columns) against ClickHouse 26.x. Your numbers will differ. Each figure
is a comparison made in one environment, and [some of them reverse in another](#perf-measuring).
</Note>
...
There are three ways to get a row out of a result, and they do not cost the same. The difference is
whether each value is *boxed* — put into a heap object — on its way to you.
...
| `ClickHouseDataReader` accessors (`GetInt32`, `GetValue`, `GetFieldValue<T>`) | Yes | This is also the path that ORMs use. |

For a 1,000,000-row read of 105 columns:
...
<Note>
`MapTo<T>` is not deprecated, and it is the correct tool when you must control the reader loop — for
example to read some rows differently, or to mix POCO reads with column access. It is the slower of
the two, so use `QueryAsync<T>` when you only need the rows.
</Note>

Two more points about the read path:

- **A column type without a fast path does not slow down the other columns.** The driver selects a
  path for each column on its own. A `Tuple` or `Map` column uses the slower path, and the scalar
  columns next to it keep the fast one.
- **ORMs (Dapper, EF Core, linq2db) use the ADO.NET reader**, so they use the boxed path. To get the
  fast path for one hot query, use `QueryAsync<T>` for that query.

---

### Inserting: batch size first {#perf-insert-batching}
...
<Warning>
**If you decreased the batch size against version 1.3.0 or earlier, test that decision again.** On
version 1.3.0, the same increase from 10,000 to 100,000 rows made inserts *slower*
(19,704 ms → 27,623 ms). The insert path no longer holds the full batch in memory before it sends it,
so large batches now help instead of hurt. The earlier tuning is now the wrong tuning.
</Warning>

On ClickHouse Cloud this advice is stronger. Small batches into a `MergeTree` table cost much more
there than on local disk, because each write goes to object storage:

| Batch size | Cloud compared with local |
|---|---:|
| 100,000 | +74% |
| 10,000 | +120% |

If you cannot control the batch size — for example when many small producers send rows independently
— use [async inserts](#async-inserts) and let the server do the batching.
...
transfer, and the two directions give opposite answers at the same distance.

**Reads: keep compression on.** This is the default. Compared with no compression, `zstd` at level 1
gave:
...
Uncompressed reads become slower as the client moves away. Compressed reads stay almost flat, because
compression changes a bandwidth problem into a CPU problem, and CPU does not increase with distance.

**Inserts: measure before you compress.** For a 250,000-row insert, compression cost time at every
distance that was measured:
...
The cost decreases as the link becomes slower, and it becomes a saving on a sufficiently slow link.
But at same-region and nearby-region distances, request compression costs time. It does still halve
the bytes, so keep it if you pay for data transfer, or if your client is far from the server.
...
<Warning>
**Do not use `GZipCompressor` at its default level for large inserts.** `GZipCompressor.Default`
compresses at `CompressionLevel.Fastest` (zlib level 1). ClickHouse reads that stream much more
slowly than the same rows at level 6 — slowly enough that a large insert can pass the 120-second
`HttpClient` timeout. If you need gzip for a proxy or a gateway, construct it with a different level:

```csharp
var options = new InsertOptions { Compressor = new GZipCompressor(CompressionLevel.Optimal) };
```

The default insert compressor is `zstd`, so this affects only code that selects gzip.
</Warning>
...
you dispose it. A reader that you do not dispose holds both until the garbage collector finalizes it,
which decreases the number of connections available in the pool.
...
**Reads do not show this effect.** Scans, typed reads, and small queries were all within measurement
noise between the two modes.

<Note>
Server GC is a throughput setting, not a latency setting. In the same measurements, Server GC spent
less than half as much total time paused (11.3% against 25.5%), but its individual pauses were longer
...
For a single small query, the network controls the time, not the driver.

A first query to a ClickHouse Cloud service one region away took 77.6 ms, against 3.0 ms to a server
on the same host. That 77.6 ms is approximately three round trips: the TCP connect, then the TLS
handshake, then the query. The driver contributed about 3 ms of it.

No driver setting makes that faster. Connection reuse does:

- Use one `ClickHouseClient` for the life of the application. It is thread-safe and is made for
  singleton use.
...
- Do not create a client for each request. Each new client with its own `HttpClient` creates a new
  connection pool, and pays for the handshake again.
...
Each figure in this section is a comparison made in one environment. Several of them **reverse** in a
different environment, so measure your own workload before you change a default.

Four rules that keep the measurements trustworthy:

1. **Do not make compression decisions from a loopback test.** Compression costs 8% on loopback and
   saves 33% one region away. A test against a server on your own machine gives you the opposite
   answer to production.
2. **On loopback, measure allocation, not elapsed time.** The client and the server compete for the
   same cores, so the elapsed time includes that competition. Allocated bytes do not.
3. **Use `ENGINE = Null` to measure the client alone.** The server accepts the rows and discards them,
   which removes the storage time from the measurement. Use a real table when you want the complete
   figure.
4. **Compare a fixed result set.** A `LIMIT n` without an `ORDER BY` returns different rows on each
   run, so each run compresses different data.

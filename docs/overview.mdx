...
| ReadBufferSize | `int` | `65536` (64 KiB) | `ReadBufferSize` | Size in bytes of the buffer that reads HTTP query responses. The driver rents the buffer from a shared pool and returns it when it disposes the reader, thus the buffer is not a per-query allocation. Increase the size to decrease buffer refills for large result sets. The driver holds one buffer for each concurrent reader, thus memory use increases with the buffer size and the number of concurrent readers. |
...
| JsonNullPathMode | `JsonNullPathMode` | `Include` | `JsonNullPathMode` | Whether a typed JSON path whose value is NULL is present in the returned `JsonObject`: `Include` (present, holding a JSON null, as the server renders it) or `Omit` (left out). Does not apply to `JsonReadMode.String`. See [JSON type](#type-map-reading-json) |
...
##### Typed paths whose value is NULL

A **typed path** is one the column declares, for example `a` in `JSON(a Nullable(Int64))`. It is
part of every row, so the server renders it with an explicit null where the row has no value for
it. A **dynamic path** exists only in the rows which have a value for it, and the server leaves it
out of a row where it is null.

`JsonNullPathMode` selects what the driver does with a typed path whose value is NULL:

- **`Include` (default)**: the path is present in the `JsonObject`, holding a JSON null. This
  matches the server, and keeps "the path is not in this row" distinguishable from "the path is
  null".

- **`Omit`**: the path is left out, as a null dynamic path already is. Where nothing else is left
  under a parent object, the parent goes too.

```csharp
// Configure omit mode via settings
var settings = new ClickHouseClientSettings("Host=localhost")
{
...
// Or via connection string
// "Host=localhost;JsonNullPathMode=Omit"
```

Reading `JSON(sub.a Nullable(Int64))`:

| Row | `Include` (default) | `Omit` |
|-----|---------------------|--------|
| `{"sub": {"a": 1, "b": 2}}` | `{"sub":{"a":1,"b":2}}` | `{"sub":{"a":1,"b":2}}` |
| `{"sub": null}` | `{"sub":{"a":null}}` | `{}` |
| `{}` | `{"sub":{"a":null}}` | `{}` |
| `{"sub": {"b": 5}}` | `{"sub":{"a":null,"b":5}}` | `{"sub":{"b":5}}` |

Two limits to keep in mind:

- The setting applies to `JsonReadMode.Binary` and `JsonReadMode.None` only. `JsonReadMode.String`
  hands back the server's own JSON text, which always spells out a typed null, so a caller who
  switches to `String` mode loses the setting.
- `Omit` leaves out a null **path**. A null *inside* a container is untouched:
  `Array(Nullable(Int32))` still reads as `[1,null,3]`, and `Map(String, Nullable(T))` still holds
  its null values.
...
An overlap declared with `Nullable` types therefore fills in one side per row and reads without
...
result is lossy: `JSON(a Int64, a.b Int64)` holding `{"a.b":7}` reads as `{"a":0}`. A path whose
parent holds a scalar or an array still throws, because a subtree cannot be placed under either.

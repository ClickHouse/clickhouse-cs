...
| Typed reader accessors (`GetInt32`, `GetInt64`, `GetDouble`, `GetGuid`, `GetDateTime`, `GetFieldValue<T>`) | **No** | Each column has storage of its own type, so the accessor reads a field. |
...
- **`IsDBNull` never materializes the value**, so a null check costs nothing on any path.

...
**Raw results are exempt.** `ExecuteRawResultAsync` hands its body to you verbatim, so the driver does not advertise the default codecs for it — it must not silently change bytes it does not consume itself, or an export would start writing compressed files. An explicit `AcceptEncoding` (at either level) still applies, and `ClickHouseRawResult.ReadDecompressedStreamAsync()` decodes the result when you want that; `ReadAsStreamAsync`, `ReadAsByteArrayAsync`, `ReadAsStringAsync` and `CopyToAsync` always return the bytes exactly as they arrived.

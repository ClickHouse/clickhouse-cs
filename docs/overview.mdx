...
`Unspecified` `DateTime`. As for a `DateTime` value, sub-second precision beyond 7 fractional digits
(100 ns) is truncated, so a `DateTime64(8)` or `DateTime64(9)` column needs a `DateTimeOffset` or a
NodaTime value to keep its last digits. An unparseable string throws `FormatException`.

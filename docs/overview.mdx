...
| AcceptEncoding | `string` | `null` | `AcceptEncoding` | `Accept-Encoding` sent with every request, replacing the codecs the driver advertises by default (`lz4, gzip, deflate`). Whatever the server answers with is decoded transparently. See [Response decompression](#response-decompression) |
...
**The server, not the client, picks the codec.** ClickHouse scans `Accept-Encoding` for tokens in its own fixed preference order — `zstd` > `br` > `lz4` > `snappy` > `gzip` > `deflate` — and ignores both the order you list them in and any q-values. So the header is a capability announcement rather than a demand, and the only way to steer the choice is which tokens you leave out. That is why `br` is *decodable but not advertised by default*: including it would make every response brotli, whose server-side cost climbs steeply with `http_zlib_compression_level`, while `lz4` is the cheapest to produce and to decode.

Because the decision is made from the response, a body is decoded whenever its `Content-Encoding` says so, whatever was requested: absent or `identity` passes through untouched, a supported codec is decoded, and anything else raises an error naming it. There is no risk of double-decoding — when the handler's `AutomaticDecompression` has already decoded a body it also strips `Content-Encoding`, so the driver sees plaintext and leaves it alone.

**Raw results keep the historical default.** `ExecuteRawResultAsync` (and the public `PostStreamAsync` / `InsertRawStreamAsync`) hand their body to you verbatim, so instead of the list above they advertise just `gzip, deflate` — exactly the codecs .NET's own `AutomaticDecompression` covers. Whatever your `HttpClient` is configured with, a raw export therefore yields the same bytes it did before this feature existed. Advertising `lz4` there would hand back a body neither the framework nor the driver decodes for you, silently turning an export into a compressed file.

...
`CommandTimeout` bounds execution server-side: a positive value in seconds is sent as the
`max_execution_time` setting, so ClickHouse stops the query once it is exceeded — with the default
`timeout_overflow_mode`, as a `ClickHouseServerException` with error code 159 (`TIMEOUT_EXCEEDED`).
Zero, the default, and negative values add no command-level limit. They do not remove a limit: a
`max_execution_time` inherited from the connection or from the server profile still applies.

```csharp
await using var command = connection.CreateCommand("SELECT count() FROM huge_table");
...
precedence; a connection-level `max_execution_time` is less specific and does not.

`CommandTimeout` is a server-side limit only — it does not cancel the client-side wait. The
connection string's `Timeout` (2 minutes by default) bounds the HTTP request, and results are
streamed, so use a `CancellationToken` on the asynchronous execute methods to abort a long read

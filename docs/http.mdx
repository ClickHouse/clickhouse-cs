...
For `InsertBinaryAsync`, the id identifies the insert itself: each batch is sent as `{QueryId}-1`, `{QueryId}-2`, and so on. The schema probe the insert runs first (`SELECT ... WHERE 1=0`) uses an id of its own, so it is never logged under your id. Because of the batch suffix, look the insert up with `WHERE query_id LIKE '<your id>-%'` rather than an equality test.

...
slug: /integrations/csharp/http
description: 'The official C# client for connecting to ClickHouse over HTTP.'
title: 'ClickHouse C# HTTP client'
doc_type: 'guide'
integration:
  - support_level: 'core'
...
| ClickHouse Type | Accepted .NET Types | Notes |
|-----------------|---------------------|-------|
| Date | `DateTime`, `DateTimeOffset`, `DateOnly`, `string`, NodaTime types | Converted to Unix days as UInt16; supported range `[1970-01-01, 2149-06-06]` |
| Date32 | `DateTime`, `DateTimeOffset`, `DateOnly`, `string`, NodaTime types | Converted to Unix days as Int32; supported range `[1900-01-01, 2299-12-31]` |
| DateTime | `DateTime`, `DateTimeOffset`, `DateOnly`, `string`, NodaTime types | See below for details; supported range `[1970-01-01, 2106-02-07 06:28:15]` UTC |
| DateTime32 | `DateTime`, `DateTimeOffset`, `DateOnly`, `string`, NodaTime types | Same as DateTime |
| DateTime64 | `DateTime`, `DateTimeOffset`, `DateOnly`, `string`, NodaTime types | Precision based on Scale parameter |
...
A `string` value is parsed culture-invariantly. A string with an offset or `Z` (`2024-01-15T14:30:00+01:00`)
preserves the instant; a string without one (`2024-01-15 14:30:00`) is wall-clock time, treated like an
`Unspecified` `DateTime`. Sub-second precision beyond 7 fractional digits (100 ns) is truncated. That
limit is not specific to a string: the write path coerces every accepted .NET type (the NodaTime types
included) to a `DateTimeOffset`, whose resolution is 100 ns, so a `DateTime64(8)` or `DateTime64(9)`
column keeps at most 7 fractional digits whichever type you write. An unparseable string throws
`FormatException`.

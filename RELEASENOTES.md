...
**New Features:**
* **Nested array parameters and bulk inserts** (issue #320): `Array(Array(T))` and deeper nestings are now supported end-to-end via parameterized queries and binary/bulk inserts. Both jagged CLR shapes (`T[][]`, `List<List<T>>`) and rectangular multidimensional CLR shapes (`T[,]`, `T[,,]`, …) are accepted on the write path. The wire format is jagged, so `GetValue` returns `T[][]`; callers who know their data is rectangular can use the standard ADO.NET `reader.GetFieldValue<T>(ordinal)` with a multidimensional `T` (e.g. `int[,]`, `byte[,,]`) and the driver materialises the column as that CLR shape in place (throws `InvalidOperationException` on ragged data). Multidimensional CLR arrays are sliced along their outermost rank at the parameter formatter and binary writer boundaries, so a `byte[,]` no longer flattens into the inner type and fails with `"Cannot convert 219 to Array(UInt8)"`.
...
* Fixed `TypeConverter.ToClickHouseType(object)` crashing when given a multidimensional CLR array value. It called `Array.GetValue(int)`, which only works for rank-1 arrays.

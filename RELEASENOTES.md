...
* Removed a per-batch allocation in the binary-insert serializers: the query line (e.g. `INSERT INTO ... FORMAT RowBinary`) that precedes each batch's rows was written via a `StreamWriter`, which allocated the writer plus a 4 KiB char buffer and matching byte buffer on every batch. It is now encoded into a pooled scratch buffer and written directly, removing one allocation per batch on both the `object[]` and POCO insert paths. As a side effect the query line is now emitted as plain UTF-8 with a `\n` terminator: the byte-order mark that `StreamWriter` prepended is gone, and the newline is `\n` on all platforms (previously `\r\n` on Windows via `Environment.NewLine`). ClickHouse accepts both, so behavior is unchanged.

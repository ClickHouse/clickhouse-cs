...
**New Features:**
* **`ClickHouse.Driver.Dapper` companion package**: opt-in NuGet that, after a single `ClickHouseDapper.Register()` call, installs Dapper type handlers for `ClickHouseDecimal`, `DateTimeOffset`, `ITuple`, `IPAddress`, and `BigInteger`, maps `DateTime` to `DbType.DateTime2`, and registers a `ClickHouseContribSqlAdapter` so `Dapper.Contrib.InsertAsync<T>` emits ClickHouse-flavoured SQL. `WHERE id IN @ids` with array parameters continues to send a single native `Array(T)` parameter — already enabled by Dapper's built-in `FeatureSupport` for `ClickHouseConnection`. Note: `Update<T>` / `Delete<T>` via Contrib are still not supported; use `ALTER TABLE … UPDATE/DELETE` directly.

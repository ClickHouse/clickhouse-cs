...
* **Data-correctness fix — `JSON` columns read with `ReadStringsAsByteArrays = true` returned base64 instead of your strings.** The setting propagates into the JSON decoder, so every string leaf was read as a `byte[]`; the decoder's type switch had no case for one, so it fell through to `JsonSerializer`, which renders a byte array as base64. `payload["event"].GetValue<string>()` returned `"aW5mbw=="` instead of `"info"` — no exception, and no visible change in the node's type. Numeric and boolean leaves were unaffected, which made the corruption easy to miss, and reading a document then writing it back **persisted the base64 text into ClickHouse**. A `Map(String, ...)` path inside `JSON` failed differently: its keys threw `InvalidCastException`.
    - **Behavioral change:** string leaves inside a `JSON` column are now always decoded as UTF-8 text regardless of `ReadStringsAsByteArrays`. This is what the decoder already did for `FixedString` paths, and it is deliberate rather than an oversight: JSON defines its strings as text (RFC 8259), so the "a ClickHouse `String` is arbitrary bytes" rationale behind the setting does not apply inside a JSON document, and `JsonValue` has no byte-array representation to expose instead.
    - **Scope:** `String` and `FixedString` paths, those wrapped in `LowCardinality` or `Nullable`, those nested in `Array` or appearing as `Map` values, and `Map` keys typed as `String`. A `Map` key typed `LowCardinality(String)` is still rejected with `NotSupportedException`, unchanged. A `byte[]` that did not originate from a string type — `Array(UInt8)` is the one that matters — is deliberately left as-is, since reinterpreting it as UTF-8 would be lossy. `Variant` and `Dynamic` paths keep their existing behaviour, because their subtype is chosen per value at read time and cannot be identified from the column type.
    - **Affected read modes:** `JsonReadMode.Binary` (the default) and `JsonReadMode.None`, which both decode the document structurally. `JsonReadMode.String` never had the bug and is untouched — there the server sends the whole document as one string, which is returned before any per-path type decoding, so the setting could never reach it. That mode is also the precedent for the decision above: the driver already returned real text there regardless of `ReadStringsAsByteArrays`, so `Binary`/`None` were the inconsistent ones.
    - `ReadStringsAsByteArrays` is **unchanged** for ordinary `String`/`FixedString` columns, and for a `Dynamic` column holding a string, where it still yields `byte[]`. Note that a `Dynamic` column holding a *`JSON` value* does get the fix, since its leaves are decoded by the JSON reader.
    - **Action required if you worked around this:** code that base64-decoded JSON string leaves itself will now double-decode — remove the workaround. Non-UTF-8 bytes in a JSON string leaf (which valid JSON cannot contain) now yield U+FFFD replacement characters rather than base64.

...
* Lower GC pressure when reading query responses: the reader no longer allocates a fresh `BufferedStream` buffer (`ReadBufferSize`, 8 KiB by default) per query. The response read buffering now rents its backing buffer from `ArrayPool<byte>` and returns it when the reader is disposed, removing one fresh allocation per query. Behavior is unchanged.

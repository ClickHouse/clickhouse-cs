...
**Bug Fixes:**
* Fixed `InsertOptions.WithColumnTypes()` and `InsertOptions.WithQueryId()` silently dropping options the caller had set. These internal "copy every property, override exactly one" helpers use a hand-maintained object initializer that had drifted from the property list: `WithColumnTypes` dropped `ParameterTypeResolver`, `ReadValueConverter` and `AcceptEncoding`; `WithQueryId` dropped `ReadValueConverter` and `AcceptEncoding` (the base `QueryOptions.WithQueryId` copied them, so the `InsertOptions` overrides had diverged from it). The user-visible effect was that an `AcceptEncoding` set on `InsertOptions` never reached a binary insert: `InsertBinaryAsync` re-derives per-batch options through `WithQueryId` (and the POCO explicit-column-types path through `WithColumnTypes`), so both the `Accept-Encoding` header override and the `enable_http_compression=1` it forces were ignored for every batch. `ReadValueConverter` (read-side) and `ParameterTypeResolver` (SQL parameters) are not consumed on the binary-insert path today, so their loss was latent, but dropping them was still a copy-correctness defect. All three copy methods now carry every property across, and the reflection-based copy tests were made non-vacuous (every property set to a non-default value) so a newly added property that is not copied fails the test.

...
* New opt-in `ClickHouse.Driver.Lz4` NuGet package adds an LZ4 codec (`Lz4Compressor`, `Content-Encoding: lz4`) for binary inserts, backed by `K4os.Compression.LZ4`. It is shipped separately so its `K4os` dependency is only pulled in when you need LZ4; install it and set `InsertOptions.Compressor = Lz4Compressor.Default`. LZ4 trades a lower compression ratio for much higher throughput than GZip/Brotli, favouring inserts where client CPU rather than bandwidth is the constraint, and it imposes the least decompression load on the ClickHouse server.

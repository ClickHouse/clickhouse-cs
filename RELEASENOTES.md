...
* Binary inserts now stream the serialized (optionally compressed) batch directly into the HTTP request body instead of first materializing the whole payload into a pooled `RecyclableMemoryStream` and seeking back to the start. This removes the transient per-batch buffer: in a 200k-row insert, peak pooled bytes dropped from ~0.5–3.75 MB to 0, scaling down with both batch size and `MaxDegreeOfParallelism` (before, up to `degree × batchSize` of compressed payload was held resident simultaneously). This reduces working-set and large-object-heap pressure for large and/or concurrent inserts; throughput and managed per-operation allocations are unchanged. Applies to `InsertBinaryAsync` (both the `object[]` and POCO overloads). Insert requests are now sent with chunked transfer-encoding (no `Content-Length`), since the payload size is no longer known up front. Serialization failures are still surfaced as `ClickHouseBulkCopySerializationException`.

...
public class TestCases
{
    ...
    private static IEnumerable<DataTypeSample> GenerateCompositeTypeSamples(DataTypeSample baseSample)
    {
        ...
        // Tuple with base type and String (System.Tuple)
        ...
        // ValueTuple with base type and String
        var valueTupleType = typeof(ValueTuple<,>).MakeGenericType(baseSample.FrameworkType, typeof(string));
        var valueTupleValue = Activator.CreateInstance(valueTupleType, baseValue, "test");
        yield return new DataTypeSample(
            $"Tuple({baseType}, String)",
            valueTupleType,
            $"tuple({baseExpr}, 'test')",
        ...
        var serverParsingFixed = TestUtilities.ServerVersion > Version.Parse("25.3");
    ...
    public static IEnumerable<DataTypeSample> GetDataTypeSamples()
    {
        ...
        yield return new DataTypeSample("Tuple(a Int32, b Int32)", typeof(Tuple<int, int>), "tuple(123, 456)", Tuple.Create(123, 456));

        // ValueTuple variants
        yield return new DataTypeSample("Tuple(Int32, String)", typeof(ValueTuple<int, string>), "tuple(42, 'hello')", (42, "hello"));
        ...
        if (TestUtilities.SupportedFeatures.HasFlag(Feature.Map))
        {
        ...
        if (TestUtilities.SupportedFeatures.HasFlag(Feature.Bool))
        {
        ...
        if (TestUtilities.ServerVersion >= Version.Parse("25.12"))
        {
            foreach (var sample in geoSamples)
        ...
        if (TestUtilities.SupportedFeatures.HasFlag(Feature.Variant))
        {
        ...
        if (TestUtilities.SupportedFeatures.HasFlag(Feature.Json))
        {
        ...
        if (TestUtilities.SupportedFeatures.HasFlag(Feature.Time))
        {
        ...
        if (TestUtilities.SupportedFeatures.HasFlag(Feature.QBit) && TestUtilities.TestEnvironment != TestEnv.Cloud)
        {

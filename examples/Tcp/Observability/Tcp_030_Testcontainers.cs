...
public static class TcpTestcontainers
{
    ...
    private static async Task<ClickHouseTcpServerInfo> HandshakeWhenReady(ClickHouseTcpClient client)
    {
        ...
        for (int attempt = 1; ; attempt++)
        {
            ...
            catch (ClickHouseTcpConnectionException) when (started.Elapsed < deadline)
            {
                // Only a connection failure is worth retrying: the listener is not accepting yet. A server

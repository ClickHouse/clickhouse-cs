...
public static class TcpTimeouts
{
    public static async Task Run()
    {
        ...
        catch (ClickHouseTcpConnectionException ex) when (ex.InnerException is TimeoutException)
        {
            Console.WriteLine($"Read timeout during server silence: {ex.Message}");
        }

        // A cancellation token limits the complete operation.
        ...
        {
            Console.WriteLine("The operation-wide cancellation timeout expired.");
        }
    }
}

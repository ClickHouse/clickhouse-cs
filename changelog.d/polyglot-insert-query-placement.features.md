* Added `InsertOptions.QueryPlacement`. Set it to `InsertQueryPlacement.Url` to send the `INSERT`
  ...
  it, instead of inside the request body, which stays the default. When the .NET runtime rejects an
  oversized URL, the error directs callers back to body placement.

...
* Pluggable **response** (read-path) decompression, so the driver decodes transport-compressed query responses itself instead of depending on what `HttpClient.AutomaticDecompression` happens to support. Set `ClickHouseClientSettings.ResponseCompressor` — or `ResponseCompression=lz4|gzip|br|none` in the connection string (for Dapper/EF Core/linq2db users), or `QueryOptions.ResponseCompressor` for a single query — and every read API (`ExecuteReaderAsync`, `ExecuteScalarAsync`, `ExecuteNonQueryAsync`, `QueryAsync<T>`, the ADO.NET command) reads plaintext rows as usual. This makes **LZ4 responses usable end-to-end** (`ResponseCompressor = Lz4Compressor.Default`): LZ4 is the cheapest codec for the ClickHouse server to produce, and .NET cannot decompress it at all. It also closes the same gap for **Brotli** — the driver's handler mask is `GZip | Deflate` only, so `AcceptEncoding = "br"` previously reached the reader still compressed and produced garbage. `IClickHouseCompressor` gained a `Decompress(Stream source, bool leaveOpen)` default interface method (implemented by `Lz4Compressor`, `GZipCompressor` and `BrotliCompressor`, throwing `NotSupportedException` by default) so custom codecs can plug into the same seam. New `ClickHouseRawResult.ReadDecompressedStreamAsync()` applies the same decoding to raw exports; the existing `ReadAsStreamAsync`/`ReadAsByteArrayAsync`/`ReadAsStringAsync`/`CopyToAsync` are unchanged and still hand back the bytes exactly as they arrived.
  - **Strictly opt-in:** with no compressor configured the default `Accept-Encoding` is unchanged (`gzip, deflate`) and nothing about existing behavior changes — LZ4 is never negotiated unless you ask for it. Setting a compressor *adds* its token to `Accept-Encoding` (it does not replace the defaults) and forces `enable_http_compression=1` on the URL, which ClickHouse requires before it honours the header. An explicit `QueryOptions.AcceptEncoding` still wins for the request header.
  - **Driven by the response, not by the request:** whether (and how) a body is decoded is decided by the response's `Content-Encoding`, because ClickHouse picks the response codec by its own fixed preference order (zstd > br > lz4 > gzip) and ignores client ordering and q-values. Absent or `identity` passes through untouched; the configured compressor's token is decoded by that compressor; `gzip`, `deflate` and `br`/`brotli` are always decoded with the BCL codec; anything else (e.g. `zstd`) now raises an error naming the codec and how to configure a decoder, instead of misparsing the body. Error bodies honour the configured compressor too, so an LZ4-compressed server error message is readable rather than a placeholder.
...
* Reading `Map` columns now pre-sizes the result `Dictionary` to the known entry count, avoiding repeated rehashing as entries are inserted (issue #478).
...
* A response body that reaches the driver still transport-compressed is now recognised up front from its `Content-Encoding` header, replacing the heuristic that sniffed for the GZip magic bytes (`0x1F 0x8B`) where the column count should be. As a result, `gzip`, `deflate` and `br` responses are decoded transparently on the read paths even when the `HttpClient` was created with `AutomaticDecompression = DecompressionMethods.None` — previously GZip threw `InvalidOperationException` telling you to enable `AutomaticDecompression`, and the other codecs were not detected at all and surfaced as a confusing type-parse failure. A codec the driver cannot decode now throws `NotSupportedException` naming the codec and listing the ways to configure a decoder. `deflate` is decoded as the zlib format (RFC 1950) that HTTP's `deflate` actually denotes and that ClickHouse emits, while still tolerating servers or proxies that send raw DEFLATE.

...
* Transport-compressed query responses are now decoded by the driver itself rather than by `HttpClient.AutomaticDecompression`, so which codecs it can accept no longer depends on the handler — notably `lz4`, which .NET cannot decode at all, and `br`, which the driver's handler mask excludes. `Accept-Encoding` is now settable client-wide via `ClickHouseClientSettings.AcceptEncoding` (connection string: `AcceptEncoding`) as well as per query, and defaults to the codecs the driver can decode. `ClickHouseRawResult.ReadDecompressedStreamAsync()` applies the same decoding to a raw export; its other members still return the body verbatim.
  - **Behavior change:** with default settings the server now answers queries with `lz4` rather than `gzip`. Raw exports are unaffected: `ExecuteRawResultAsync` does not advertise the default codecs, so its body stays uncompressed unless `AcceptEncoding` is set explicitly. Set `AcceptEncoding = "gzip"` or `"identity"` to opt out.

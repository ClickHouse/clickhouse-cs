...
* Transport-compressed query responses are now decoded by the driver itself rather than by `HttpClient.AutomaticDecompression`, so which codecs it can accept no longer depends on the handler — notably `lz4` and `br`, which .NET's `GZip | Deflate` mask did not cover. The handler the driver builds no longer sets `AutomaticDecompression` at all, so an explicit `Accept-Encoding` now reaches the server exactly instead of silently gaining `gzip`/`deflate`: .NET adds every algorithm in the mask to the outgoing header at send time, which meant `AcceptEncoding = "identity"` did not actually disable wire compression and `"deflate"` did not actually select deflate. `Accept-Encoding` is now settable client-wide via `ClickHouseClientSettings.AcceptEncoding` (connection string: `AcceptEncoding`) as well as per query, and defaults to the codecs the driver can decode. `ClickHouseRawResult.ReadDecompressedStreamAsync()` applies the same decoding to a raw export; its other members still return the body verbatim.
  - **Behavior change — the default response codec is now `lz4`** (previously `gzip`). Compression itself was already on by default; only the codec changed. To choose otherwise, set `AcceptEncoding` to `"gzip"`, `"br"` or `"identity"` — client-wide (`ClickHouseClientSettings.AcceptEncoding`), in the connection string (`AcceptEncoding=`), or per query (`QueryOptions.AcceptEncoding`).
  - **Breaking — verbatim bodies are no longer decoded for you.** `ExecuteRawResultAsync`, `PostStreamAsync` and `InsertRawStreamAsync` hand back exactly what the server sent, since the handler no longer decodes anything. **If you set `AcceptEncoding` explicitly, these now return compressed bytes where they previously returned plaintext** — call `ClickHouseRawResult.ReadDecompressedStreamAsync()` to decode, or read `ContentEncoding` and decode yourself. With nothing configured they advertise no codec, so the body is plaintext: unchanged on the driver's own handler, though a caller-supplied `HttpClient` with `AutomaticDecompression = DecompressionMethods.None` previously received `gzip` bytes here.
...
* A response that reaches the driver still transport-compressed is now recognised from its `Content-Encoding` header instead of a GZip magic-byte sniff, so `gzip`, `deflate` and `br` are decoded even when the `HttpClient` was built with `AutomaticDecompression = DecompressionMethods.None`, and a codec the driver cannot decode (e.g. `zstd`) throws `NotSupportedException` naming it instead of surfacing as a confusing type-parse failure. HTTP `deflate` is decoded as the zlib format (RFC 1950) that ClickHouse actually emits, while still tolerating raw DEFLATE.

...
**New Features:**
* Added `JsonNullPathMode` (connection string `JsonNullPathMode=Omit`, or `ClickHouseClientSettings.JsonNullPathMode`), which selects whether a typed `JSON` path holding NULL is present in the returned `JsonObject`. `Include`, the default, keeps the behavior added in v1.4.0: the path is present holding a JSON null, matching the server, so that "the path is not in this row" stays distinguishable from "the path is null". `Omit` restores the pre-1.4 shape, where such a path is left out — and with it a parent object which has nothing else left under it, so `JSON(sub.a Nullable(Int64))` holding a null reads back as `{}` rather than `{"sub":{"a":null}}`. The setting applies to `JsonReadMode.Binary` and `JsonReadMode.None`; `JsonReadMode.String` returns the server's JSON text, which always spells out a typed null. It drops a null *path* only — a null inside an `Array` or a `Map` is untouched in both modes. Note that `Omit` is not exactly the pre-1.4 behavior for a column with overlapping typed paths, which used to return silently wrong data and now throws in both modes (see below).

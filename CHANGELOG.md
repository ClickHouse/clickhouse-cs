...
* Fixed HTTP query parameters silently corrupting a `byte[]` bound to a `String` column — the literal text `System.Byte[]` was sent instead of the payload — and being unable to bind a `TimeOnly` to a `Time`/`Time64` column at all (it threw `InvalidCastException`/`ArgumentException`). `byte[]` and `ReadOnlyMemory<byte>` are now escaped byte-for-byte on the HTTP parameter path (so payloads that are not valid UTF-8 round-trip losslessly instead of being mangled by UTF-8 decoding), mirroring the binary write path; `TimeOnly` binds to `Time`/`Time64` (and to the binary write path); and a `TimeOnly` with no type hint now infers as `Time64(7)`, matching `TimeSpan` (issue #483).

...
* Faster scalar reads from query responses, from two changes to the reader's stream chain.
  - `PooledReadBufferStream` and `ExceptionTagAwareStream` did not override `Stream.Read(Span<byte>)`, so every fixed-size read `BinaryReader` issues (4 bytes for `Int32`, 8 for `Int64`/`Float64`, and so on) fell through to the base `Stream.Read(Span<byte>)` implementation, which rents an array from `ArrayPool<byte>` and copies through it on *every* call. `PeekableStreamWrapper` already overrides the span read for exactly this reason, but it sits directly on top of these two, so the rent-and-copy was reintroduced one layer further down. Both streams now implement the span read directly.
  - Mid-stream exception detection (`ExceptionTagAwareStream`, active only when the server sends an `X-ClickHouse-Exception-Tag` response header) recorded recent bytes into its ring buffer from *above* the read buffer, so it observed one read per scalar the reader decoded and copied a few bytes each time. It now records from below the read buffer, where it sees one read per buffer refill instead; every such refill is at least as large as the ring capacity, so it takes the pre-existing single-copy shortcut — one 4 KiB copy per 64 KiB read rather than millions of few-byte copies.
  - Measured over a 32 MiB payload read through the real reader stream chain, per-value cost drops from 29.7 ns to 8.8 ns for `Int32` (~3.4x), 29.0 ns to 11.1 ns for `Int64` (~2.6x) and 29.5 ns to 10.0 ns for `Float64` (~3.0x). When the exception-tag stream is in the chain the gain is larger — `Int32` 40.2 ns to 8.8 ns (~4.6x) — and its residual cost is now within measurement noise of not having it in the chain at all.
  - Behavior is unchanged, including the documented short-read contract and mid-stream exception detection. The exception ring buffer now retains the last bytes *received* rather than the last bytes *consumed*, which is equivalent for its purpose: it is only consulted after an `EndOfStreamException`, by which point the transport is drained and the server's trailing marker has been recorded either way.

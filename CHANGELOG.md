...
* Fixed string values inside a `JSON` column being silently base64-encoded when `ReadStringsAsByteArrays = true`. The setting propagates into the JSON decoder, so every string leaf was read as a `byte[]`; the decoder's type switch had no case for one, so it fell through to `JsonSerializer`, which renders a byte array as base64. `payload["event"].GetValue<string>()` returned `"aW5mbw=="` instead of `"info"` — no exception, and no visible change in the node's type. Numeric and boolean leaves were unaffected, which made the corruption easy to miss, and reading a document then writing it back persisted the base64 text into ClickHouse. A `Map(String, ...)` path inside `JSON` failed differently: its keys threw `InvalidCastException`. **Behavioral change:** string leaves inside a `JSON` column are now always decoded as UTF-8 text regardless of `ReadStringsAsByteArrays`, which is what the decoder already did for `FixedString` paths — JSON defines its strings as text (RFC 8259) and `JsonValue` has no byte-array representation. This covers `String` and `FixedString` paths, those wrapped in `LowCardinality` or `Nullable`, those nested in `Array` or as `Map` values, and `Map` keys typed as `String`. A `byte[]` that did *not* come from a string type — `Array(UInt8)`, for instance — is deliberately left alone, and `Variant`/`Dynamic` paths keep their existing behaviour because their subtype is only known per value at read time. This applies to `JsonReadMode.Binary` (the default) and `JsonReadMode.None`; `JsonReadMode.String` never had the bug, since it returns the whole document as one string before any per-path type decoding. `ReadStringsAsByteArrays` is unchanged for ordinary `String`/`FixedString` columns, and for a `Dynamic` column holding a string, where it still yields `byte[]`. If you decoded that base64 yourself as a workaround, remove it.

...
* Fixed `ClickHouseClient.InsertRawStreamAsync` and the stream overloads of `ClickHouseClient.PostStreamAsync`/`ClickHouseConnection.PostStreamAsync` disposing the caller-supplied `Stream`. The stream was handed to a `StreamContent` that the request message disposed along with itself, closing a stream the caller still owned (and, when `InsertRawStreamAsync` failed, disposing it twice). Caller-supplied streams are now read but never disposed, so the same stream can be rewound and reused across calls; disposing it remains the caller's responsibility.

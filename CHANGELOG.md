...
* Fixed `ClickHouseClient` never disposing the query's `HttpResponseMessage` on the paths that fully consume it: `ExecuteNonQueryAsync`, binary inserts (`InsertBinaryAsync`), and any request that fails (a server error or transport failure in `PostSqlQueryAsync`/the streamed insert path). The response — and with it the pooled HTTP connection — is now released as soon as the client is done with it instead of waiting for finalization, which degraded `HttpClient` connection pooling under load. Paths that hand the response to the caller are unchanged: `ExecuteReaderAsync`/`ExecuteRawResultAsync` still release it when the returned reader/result is disposed, and `InsertRawStreamAsync`/`PostStreamAsync` still return the `HttpResponseMessage` for the caller to dispose.

...
* Faster reading of `Array` columns with common element types. `ArrayType.Read` now builds a strongly-typed `T[]` and stores each element through the typed array indexer, instead of allocating the array via `Array.CreateInstance` and storing every element through a reflection-based `Array.SetValue`. This covers leaf element types — the integer, floating-point, `Bool`, `Decimal`, `DateTime`/`Date`/`DateTime64`, `UUID`, `String`, `Enum`, and `FixedString` types, plus their `Nullable` forms — while composite/nested elements (`Array`, `Tuple`, `Map`, `Nested`), big integers, `ClickHouseDecimal` (big-decimal mode), and IP addresses keep the existing reflection path. In an isolated read benchmark (100k arrays, min-of-25 passes) per-array read time dropped ~12–58% depending on element type (e.g. `Array(Nullable(Int32))` ≈ −58%, `Array(Int32)` ≈ −33%, a 3-element `Array(Int32)` ≈ −41%), with identical allocations. Behavior is unchanged, including the exact returned array type (e.g. `int?[]`, `int[][]`) and `DBNull`→`null` handling for nullable elements.

...
**Bug Fixes:**
* Fixed `{name:Type}` parameter type hints being silently dropped when the query contains an earlier `{` that is not a type hint, such as `{name}` without a type or an unterminated `{name:Type`. The scans for the name/type separator and for the end of the type definition were unbounded, so they latched onto a later parameter's colon or closing brace, recorded a garbage parameter name and skipped past the real hint, which then fell back to CLR-type inference (issue #510). Parameter names are now also required to be identifiers, matching the server.

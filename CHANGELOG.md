...
**Bug Fixes:**
* Fixed reading `Enum8`/`Enum16` columns through the numeric `ClickHouseDataReader` accessors (issue #439). `GetByte`, `GetSByte`, `GetInt16`, `GetInt32`, `GetInt64`, `GetUInt16`/`GetUInt32`/`GetUInt64`, and `GetFieldValue<T>` for an integer `T` previously threw `InvalidCastException` on an `Enum8`/`Enum16` (or `Nullable(Enum...)`) column because the value is materialized as its string label. They now return the underlying integer ordinal that arrived on the wire. `GetString`, `GetValue`, and `GetFieldValue<string>` still return the label (unchanged). An ordinal that does not fit the requested integer type throws `OverflowException` rather than truncating silently. The integer ordinal reflects the raw wire value and is independent of a configured `IReadValueConverter` (which continues to transform the string label seen by `GetString`/`GetValue`/`GetFieldValue<string>`).

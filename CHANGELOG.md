...
* Fixed string values inside a `JSON` column being silently returned as base64 when `ReadStringsAsByteArrays = true` (`GetValue<string>()` gave `"aW5mbw=="` instead of `"info"`), which a read-then-write round trip then persisted into ClickHouse; `Map(String, ...)` keys threw `InvalidCastException`. **Behavioral change:** strings inside a `JSON` column are now always decoded as UTF-8 text regardless of the setting, matching `FixedString` paths and `JsonReadMode.String` — remove any base64 workaround. The setting is unchanged for ordinary `String`/`FixedString` columns.

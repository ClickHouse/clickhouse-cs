...
* Reduced rehashing when reading `Map` columns: `MapType.Read` now constructs the result `Dictionary<TKey, TValue>` pre-sized to the map's known key/value-pair count instead of growing it from empty, avoiding the repeated internal resize/rehash cycles as entries are inserted. This mirrors the existing `Array` read path, which already allocates its result with the known length. Behavior is unchanged — the returned keys and values are identical (issue #478).

...
* Reduced allocations in POCO binary inserts (issue #505): value-type properties are now written through a compiled, box-free delegate instead of boxing each value into `object` and unboxing it inside the type's `Write`. Coverage is broad: the fixed-width scalar value types (`Int8`–`Int64`, `UInt8`–`UInt64`, `Float32`/`Float64`, `BFloat16`, `Bool`), `UUID` (`Guid`), the `DateTime`/`Date` family (`DateTime`, `DateTime32`, `DateTime64`, `Date`, `Date32`) from `DateTime`, `DateTimeOffset`, or `DateOnly` properties, `Decimal*` (`decimal`/`ClickHouseDecimal`), `Int128`/`Int256`/`UInt128`/`UInt256` (`BigInteger`), `Time`/`Time64` (`TimeSpan`), and `Nullable(T)` wrappers around any of these (a non-null value writes its underlying without boxing; a null writes just the marker byte). For integer, float, `Decimal`, and `BFloat16` columns the fast path also covers any other numeric value-type property and C# enums (e.g. an `int` property on a `UInt64` column, or an `enum` on an `Int8` column) via a box-free `Convert.ToXxx`; the emitted bytes and any overflow are identical to the boxed path. This removes one heap allocation per matched value per row on the `RowBinary` insert path (the common `client.InsertBinaryAsync(table, pocos)` case). In a 500k-row insert of a 3-column POCO (`Int64`, `String`, `Float64`), managed allocations dropped ~37% (62.4 MB → 39.5 MB) and Gen2 collections were eliminated. `String` properties also use the compiled delegate, but only to skip virtual dispatch — a `string` reference is never boxed, so there is no allocation to remove there. Reference-typed values other than `String` (`FixedString`, `Array`, `Tuple`, `Map`, `Nested`, `IPv4`/`IPv6`, enums stored as strings, …) are never boxed by the getter and so keep the existing path; any value type without a matching conversion also falls back — output is byte-identical and behavior is unchanged. The `RowBinaryWithDefaults` path is unchanged.

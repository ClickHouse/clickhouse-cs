...
* Reduced allocations in POCO reads (`client.QueryAsync<T>(...)`): eligible rows are now materialized by a compiled per-column delegate that reads each value straight from the response stream into the target property, bypassing the reader's shared `object[]` row buffer and the boxing/unboxing setter. This removes both the box (when decoding) and the unbox (when assigning) per value-type property per row. In a 500k-row read of a 3-column POCO (`Int64`, `String`, `Float64`), managed allocations dropped ~38% (61.1 MB → 38.2 MB) and Gen0 collections roughly halved. The fast path covers all scalar/value column types — the integer and float families, `Bool`, `BFloat16`, `Decimal`, `Int128`/`Int256`/`UInt128`/`UInt256`, `Date`/`Date32`/`DateTime`/`DateTime64`, `Time`/`Time64`, `UUID`, `IPv4`/`IPv6`, `Enum8`/`Enum16` — plus `String`/`FixedString`, transparently through `Nullable(T)` and `LowCardinality(T)`. Composite/polymorphic columns (`Array`, `Tuple`, `Map`, `Nested`, `Variant`, `Dynamic`, `JSON`) fall back per-column to the existing boxed read, so a POCO mixing fast and composite columns still benefits on the fast ones. A registered read-value converter or an unregistered type falls back entirely. `MapTo<T>` and the ADO `GetValue`/`GetFieldValue<T>` accessors are unchanged.
* POCO reads via `QueryAsync<T>` can now materialize a column into more than one CLR type, chosen by the property: a `DateTime`/`DateTime64`/`Date` column into `DateTime`, `DateTimeOffset`, or `DateOnly`; a `String`/`FixedString` column into `string` or `byte[]`; a `Decimal` column into `decimal` or `ClickHouseDecimal`; and (on .NET 8+) an `Int128`/`UInt128` column into the native `System.Int128`/`System.UInt128` in addition to `BigInteger`. These extended bindings are supported on the `QueryAsync<T>` fast path only; `MapTo<T>` keeps its strict exact-framework-type rules.
* `Map(K,V)` columns can now be read into and written from a `List<KeyValuePair<K,V>>` or `KeyValuePair<K,V>[]` POCO property, in addition to `Dictionary<K,V>`. On read (`QueryAsync<T>`), the list/array form preserves the on-wire entry order and any duplicate keys — which a `Dictionary` silently collapses — since a ClickHouse `Map` is physically an ordered array of key/value pairs. On binary insert, a `Map` column now accepts an ordered collection of `KeyValuePair<,>` as well as an `IDictionary`. `Dictionary<K,V>` continues to work unchanged on both paths.

...
* Reduced per-row allocations when reading `Tuple(...)` columns with up to 7 elements. Materializing a tuple row previously allocated two intermediate `object[]` buffers and invoked the tuple constructor reflectively through `Activator.CreateInstance` for every row. The read path now constructs the `System.Tuple<...>` directly through a compiled factory, passing the elements as positional arguments so neither `object[]` is allocated. The factory is cached per tuple shape (keyed by the tuple CLR type), so it is built once per distinct tuple type for the whole process rather than per row — including on the `Dynamic` read path, where a fresh type descriptor is decoded for every value. In a 200k-row read benchmark (single tuple column, `GetValue`), managed allocations per read dropped `Tuple(UInt64, String)` −78% (78.6 MB → 17.6 MB), `Tuple(Int32, Int32, Int32)` −78% (88.6 MB → 19.9 MB), `Tuple(Nullable(Int32), String)` −83% (98.1 MB → 16.7 MB), and a 7-element `Tuple(Int32, ...)` −70% (137.4 MB → 41.2 MB), cutting Gen0 collections 4-5× and roughly halving end-to-end read time; a `Dynamic`-wrapped tuple improved −37% (161.0 MB → 101.5 MB). Tuples with more than 7 elements have no `System.Tuple<...>` arity and keep the existing `LargeTuple` path unchanged (byte-identical allocation in the same benchmark). Behavior is unchanged: the value is still the same `System.Tuple<...>` CLR type (never a `ValueTuple`), `NULL`s inside `Nullable(...)` elements are still surfaced as `null`, and nested composite elements (arrays, maps, tuples) are unaffected.
